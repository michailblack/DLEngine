...
void mainGS(triangle VertexOutput input[3], inout TriangleStream<GeometryOutput> output)
{
    for (uint faceIndex = 0; faceIndex < 6; ++faceIndex)
    {
        for (uint vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
        {
            ...
            outputVertex.v_WorldPos = input[vertexIndex].v_WorldPos;
            outputVertex.v_SpherePosition = input[vertexIndex].v_SpherePosition;
            outputVertex.v_TexCoords = input[vertexIndex].v_TexCoords;
            outputVertex.v_RenderTargetArrayIndex = faceIndex;
            outputVertex.v_IncinerationFactor = input[vertexIndex].v_IncinerationFactor;
            outputVertex.v_CurrentSphereRadius = input[vertexIndex].v_CurrentSphereRadius;
            outputVertex.v_EpsilonSphereRadius = input[vertexIndex].v_EpsilonSphereRadius;
